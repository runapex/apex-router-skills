...
description: Use PROACTIVELY the moment a task has many layers — multiple dependent steps, unknowns that could change the approach, debugging where the first theory may be wrong, or anything needing verification before handoff. Also use when a task keeps failing or stalling, or when the user says "disciplined execution", "rigorous mode", "slow down and do this right", "think this through first", or (legacy) "fable mode" / "use the Fable method".
---

# Disciplined Execution

A rigorous working discipline, written down so any model can run it — a five-gate loop for scoping, gathering evidence, attacking your own answers, verifying, and reporting. Run this loop on any capable model and the output gets noticeably more reliable on planning, debugging, and review. (Originally distilled from Fable 5's working method; the discipline is model-agnostic.)
...
- It stacks with task-specific skills (verify-claims, cross-validate). Those are the "how to check" tools; this is the discipline of when to reach for them.
